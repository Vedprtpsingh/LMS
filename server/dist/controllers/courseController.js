...
export const getCourses = async (req, res) => {
    try {
        const { search, category, level, status, instructorId, sort = 'latest', page = '1', limit = '20' } = req.query;
        let query = supabase
            .from('courses')
            .select(`
        id,
...
export const createCourse = async (req, res) => {
    try {
        const { title, description, categoryId, level, language, price, thumbnailUrl } = req.body;
        if (!title) {
